...
int SPEED_1 = 120;        // If too fast, the water in the tank will spill out!
int SPEED_2 = 100;
int SPEED_3 = 100;
...
void setup() {
  ...
  Serial.begin(9600);
  //BTserial.begin(9600);      // NOTE: Higher the buad rate, lower will be the BT range, 
  ...
  demoServo.attach(servoPin);           // Attaching the servo and then detaching it in hope that it would remove the jitter WHENEVER the servo 
  ...
  demoServo.write(servoAngleDefault);   // movement. Now it 0.2 seconds. Remember that 'servo.write' needs some time to work, so we need some delay between
...
void loop() 
{
  // put your main code here, to run repeatedly:
    ...
    //int dataBT = BTserial.read();          //////////////////// TRUCK MOVEMENT AND OTHER FEATURES USING BLUETOOTH
    int dataBT = Serial.read();
  ...
  Serial.println(flameVal_Center_R, 3);
  
  Serial.println("Center Left");
  Serial.println(flameVal_Center_L, 3);

  Serial.println("Left");
  Serial.println(flameVal_Left, 3);
  
  Serial.println("Differece");
  Serial.println(abs(flameVal_Center_R - flameVal_Center_L), 3);
